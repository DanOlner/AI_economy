...
This analysis is built on two sources to create two distinct AI measures, each with two sub-indices. First, we use data produced by [Felten et al](https://sms.onlinelibrary.wiley.com/doi/full/10.1002/smj.3286){target="_blank"} (2021)[^1]. They created two indices - an 'AI Industrial Exposure' (AIIE) index that assigns exposure values to U.S. sectors, and an 'AI Occupational Exposure' (AIOE) index that does the same for occupation codes. Each are normalised, so that their centre value is zero - effectively, they give a relative measure of 'less to more exposed' across the full range of industrial and occupation categories. (The original data is available [here](https://github.com/AIOE-Data/AIOE){target="_blank"}.)

[^1]: Felten, E., Raj, M., Seamans, R., 2021. Occupational, industry, and geographic exposure to artificial intelligence: A novel dataset and its potential uses. Strategic Management Journal 42, 2195–2217.

The U.S. industry data (NAICS) was 'cross-walked' to U.K Standard Industrial Classification data from the ONS, matching against what were the most appropriate SIC codes. These varied from 2 to 5 digit codes. An algorithm was then used to apply AIIE values to firm data from UK Companies House that 'cascaded' the most appropriate AIIE value at the most granular level possible, by applying 5 digit values where possible, then 4 digit, then 3, then 2. Firms in Companies House use up to four different 5 digit SIC codes, though most use only one. The AIIE was averaged for any firms using more than one. This produced specific AIIE values for each UK firm. When applying the Companies House AIIE per-firm values to the analysis, these are always weighted by the number of employees per firm if, for example, producing a weighted average for UK geographies or sectors.
...
#Save copy for export, put GM LAs at top
write_csv(
  LAsummary2 %>% 
    mutate(
      is_gm = localauthority_name %in% gmlas
    ) %>% 
    relocate(is_gm, .after = localauthority_name) %>% 
    relocate(AIIEfinal_weightedmean, .before = augment_morethan_replace_prob_weightedmean) %>% 
    arrange(-is_gm) %>%  
    select(localauthority_name:augment_morethan_replace_prob_weightedmean),
  '../../local/data/la_AI_summarymeans_forGB.csv'
)
